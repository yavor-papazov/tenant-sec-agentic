...
def build_assessment_yaml(
    *,
    ...
    recommended_services: dict[str, int] | None,
...
def validate_internal_assessment(data: dict[str, Any]) -> list[str]:
    errs: list[str] = []
    for k in ("control", "provider", "recommended_score", "status"):
        if k not in data:
            errs.append(f"missing {k}")
    score = data.get("recommended_score")
    if score not in (0, 1, 2, 3, "mixed"):
...
def control_entry_for_provider_schema(
    ...
    if recommended_score == "mixed":
        ...
        for sid, sc in (recommended_services or {}).items():
            ev = ""
            if isinstance(src.get(sid), dict):
                ev = str(src[sid].get("evidence") or "")
            services_out[sid] = {"score": int(sc), "evidence": ev or "See assessor evidence."}
        summ = str(assessor.get("evidence") or "")[:500] or "Per-service maturity varies."
        return {
            "score": "mixed",
            "summary": summ,
    ...
    ev = str(assessor.get("evidence") or "")
    if not ev:
        ev = "No evidence captured."
    return {
...
def render_control_html(
    ...
    rec_label = "MIXED" if rec == "mixed" else f"L{rec}"
...
def render_index_html(
    ...
    total = len(rows)
    dist = {"L0": 0, "L1": 0, "L2": 0, "L3": 0, "mixed": 0}
    for r in rows:
        s = r.get("score")
        if s == "mixed":
            dist["mixed"] += 1
...
<p>Controls: {total} · Distribution: L0 {dist['L0']}, L1 {dist['L1']}, L2 {dist['L2']}, L3 {dist['L3']}, mixed {dist['mixed']}</p>
